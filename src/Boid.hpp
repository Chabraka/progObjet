...
public:
    // Constructors
    Boid()
        : _center(glm::vec3(1.0f)), _radius(1.0f), _speed(glm::vec3(1.0f)), _acceleration(glm::vec3(1.0f)){};
    Boid(glm::vec3 center, glm::float32 radius, glm::vec3 speed, glm::vec3 acceleration)
        : _center(center), _radius(radius), _speed(speed), _acceleration(acceleration){};

    // Draw
    void drawBoid(const p6::Shader* shader, glm::mat4 ProjMatrix, GLuint vao);
