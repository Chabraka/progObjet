...
public:
    ...
    Model(const char* obj_path, const char* image_path, const p6::Shader* shader)
        : _radius(1.0f), 
          ...
          _speed(glm::vec3(1.0f)), 
          _acceleration(glm::vec3(1.0f)),
          _objrenderer(ObjRenderer(obj_path, image_path, shader))
          {};
    Model(const char* obj_path, const char* image_path, const p6::Shader* shader, 
          glm::float32 radius, glm::vec3 center, glm::vec3 speed, glm::vec3 acceleration)
        : _radius(radius), 
          _center(center), 
          _speed(speed), 
