...
/* ----- Constructor ----- */
Boids::Boids(Parameters& params, float floor_low_medium, float floor_medium_high, const p6::Shader* shader, glm::vec3 diffuse, glm::vec3 specular, float shininess){
...
/* ----- Draw ----- */
void Boids::drawBoids(glm::mat4 ProjMatrix, glm::mat4 ViewMatrix, Parameters& params, float dt, glm::vec3 cam_position, glm::vec3 lightpos, glm::vec3 lightIntensity)
{
    for (int j = 0; j < params.BOID_NB; j++)
    {
        ...
        this->drawBoid(&_boids[j],ProjMatrix, ViewMatrix, cam_distance, lightpos, lightIntensity);
        _boids[j].updatePosition(params, dt);
    }
}

void Boids::drawBoid(Boid *boid, glm::mat4 ProjMatrix, glm::mat4 ViewMatrix, float cam_distance, glm::vec3 lightpos, glm::vec3 lightIntensity)
{
    ...
    this->renderer.low_renderer.shader->set("uLightPos_vs", lightpos);
    this->renderer.low_renderer.shader->set("uLightIntensity", lightIntensity);
