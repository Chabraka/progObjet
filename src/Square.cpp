...
void Square::updateAcc(std::vector<Square> boids, unsigned int i)
{
    ...
    for (unsigned int j = 0; j < boids.size(); j++)
    {
        ...
        float factorAttraction = 0.0004;
        glm::vec2 attraction = direction*factorAttraction;


        float factorRepulsion = -0.0002;
        glm::vec2 repulsion = direction*(1/(distance*distance))*factorRepulsion;


        acc += attraction+repulsion;

        if (distance < 0.7) {  // distance minimale à régler.
    ...
    if (numspeedboids!=0){
        ...
        acc = acc + BoidsSpeedMean*(float)1.; // paramètre à régler.
    }
